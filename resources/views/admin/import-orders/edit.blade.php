...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">Chỉnh sửa Phiếu Nhập: {{ $importOrder->code }}</h1>
            </div>
        </div>
    </div>
</div>
...
<section class="content">
    <div class="container-fluid">
        <form action="{{ route('admin.import-orders.update', $importOrder) }}" method="POST" id="importOrderForm">
            @csrf
            @method('PUT')
                ...
                <div class="card-body p-4">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select name="source" id="source" class="form-select shadow-sm @error('source') is-invalid @enderror" required style="border-radius: 8px;">
                                    <option value="supplier" {{ old('source', $importOrder->source) == 'supplier' ? 'selected' : '' }}>Từ nhà cung cấp</option>
                                    <option value="transfer" {{ old('source', $importOrder->source) == 'transfer' ? 'selected' : '' }}>Điều chuyển từ kho khác</option>
                                    <option value="return" {{ old('source', $importOrder->source) == 'return' ? 'selected' : '' }}>Trả hàng</option>
                                    <option value="other" {{ old('source', $importOrder->source) == 'other' ? 'selected' : '' }}>Khác</option>
                                </select>
                                @error('source')
                        ...
                        <div class="col-md-6">
                            <div class="mb-3" id="supplier-field">
                                ...
                                <select name="supplier_id" class="form-select shadow-sm @error('supplier_id') is-invalid @enderror" style="border-radius: 8px;">
                                    ...
                                    @foreach($suppliers as $supplier)
                                        <option value="{{ $supplier->id }}" {{ old('supplier_id', $importOrder->supplier_id) == $supplier->id ? 'selected' : '' }}>
                                            {{ $supplier->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3" id="warehouse-field" style="display: none;">
                                ...
                                <input type="text" name="from_warehouse" class="form-control shadow-sm @error('from_warehouse') is-invalid @enderror" value="{{ old('from_warehouse', $importOrder->from_warehouse) }}" placeholder="Nhập tên kho..." style="border-radius: 8px;">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label class="form-label fw-semibold">
                                    <i class="bi bi-person-check me-1 text-primary"></i>Giao cho nhân viên
                                </label>
                                <select name="assigned_to" class="form-select shadow-sm @error('assigned_to') is-invalid @enderror" style="border-radius: 8px;">
                                    <option value="">Chọn nhân viên</option>
                                    @foreach($staff as $s)
                                        <option value="{{ $s->id }}" {{ old('assigned_to', $importOrder->assigned_to) == $s->id ? 'selected' : '' }}>
                                            {{ $s->name }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="notes" class="form-control shadow-sm @error('notes') is-invalid @enderror" rows="2" placeholder="Nhập ghi chú..." style="border-radius: 8px;">{{ old('notes', $importOrder->notes) }}</textarea>
                        @error('notes')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        @foreach($importOrder->items as $index => $item)
                            <div class="item-row row mb-3">
                                ...
                                <div class="col-md-2">
                                    ...
                                    <input type="number" name="items[{{ $index }}][unit_price]" class="form-control price-input" min="0" step="0.01" value="{{ $item->unit_price }}" required>
                                </div>
                                <div class="col-md-2">
                                    <label class="form-label">Thành tiền</label>
                                    <input type="text" class="form-control total-input" readonly value="{{ number_format($item->total_price, 0, ',', '.') }}">
                                </div>
                                <div class="col-md-2">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div>
                            <button type="submit" class="btn btn-primary px-4">
                                <i class="bi bi-check-circle me-1"></i>Cập nhật phiếu nhập
                            </button>
                            <a href="{{ route('admin.import-orders.show', $importOrder) }}" class="btn btn-secondary px-4">
...
@push('scripts')
<script>
let itemIndex = {{ $importOrder->items->count() }};
