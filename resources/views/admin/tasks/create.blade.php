        ...
        <div class="card shadow-sm">
            <div class="card-header bg-gradient-primary">
                <h3 class="card-title mb-0 text-white">
                    <i class="bi bi-plus-circle me-2"></i>Phân công Công việc
                </h3>
            </div>
            <div class="card-body p-4">
                <form action="{{ route('admin.tasks.store') }}" method="POST">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label class="form-label">Phân công theo chức vụ</label>
                                ...
                                <small class="text-muted">Hệ thống sẽ tự động chọn nhân viên phù hợp dựa trên chức vụ và khối lượng công việc</small>
                                @error('position')
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label class="form-label">Hoặc chọn nhân viên cụ thể</label>
                                ...
                                <small class="text-muted">Nếu chọn nhân viên cụ thể, hệ thống sẽ bỏ qua phân công tự động</small>
                                @error('assigned_to')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select name="type" class="form-select @error('type') is-invalid @enderror" required>
                                    ...
                                    <option value="inventory_check" {{ old('type') == 'inventory_check' ? 'selected' : '' }}>Kiểm kê</option>
                                    <option value="picking" {{ old('type') == 'picking' ? 'selected' : '' }}>Soạn hàng</option>
                                    <option value="stock_report" {{ old('type') == 'stock_report' ? 'selected' : '' }}>Báo cáo tồn</option>
                                </select>
                                @error('type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select name="related_order_type" class="form-select @error('related_order_type') is-invalid @enderror">
                                    ...
                                    <option value="import_order" {{ old('related_order_type') == 'import_order' ? 'selected' : '' }}>Phiếu nhập</option>
                                    <option value="export_order" {{ old('related_order_type') == 'export_order' ? 'selected' : '' }}>Phiếu xuất</option>
                                </select>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror" rows="4" placeholder="Nhập mô tả công việc hoặc click 'Điền mặc định' để tự động điền...">{{ old('description') }}</textarea>
                        <small class="text-muted">Nếu để trống, hệ thống sẽ tự động điền mô tả mặc định dựa trên loại công việc.</small>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label class="form-label">ID phiếu liên quan (nếu có)</label>
                                <input type="number" name="related_order_id" class="form-control @error('related_order_id') is-invalid @enderror" value="{{ old('related_order_id') }}">
...
// Auto-fill description when type changes (if description is empty)
document.querySelector('select[name="type"]').addEventListener('change', function() {
    const descriptionField = document.getElementById('description');
    if (!descriptionField.value.trim() && defaultDescriptions[this.value]) {
        descriptionField.value = defaultDescriptions[this.value];
    }
});
</script>
@endpush
